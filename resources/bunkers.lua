...
------------------------------------------------------------------------
 ownBunker = true
--the animation only works with the one bunker
local bunkerCoords = {
...
lestersSweatshop = GetInteriorAtCoordsWithType(706.8047, -965.5617, 29.4179, GetHashKey("v_sweat"))
--RequestCutscene("HEIST_INT", 8)
 --Citizen.InvokeNative(0x9C27559FFF13554A, "MP_1", 0, 1)
  --RegisterEntityForCutscene(PlayerPedId(), "MP_1", 0, 0, 64)
  --StartCutscene(0)
  --NetworkFadeInEntity(PlayerPedId(), 1, 1)
 LoadInterior(lestersSweatshop)
-----------------------------------------------------------------------
...
--this is supposed to put the playerID's and names in an array
Citizen.CreateThread(function()
...
--this makes player unarmed while inside the bunker	 
...
function createMarker()
 ...
 while true do
--this makes player unarmed while inside the bunker	
	 if GetInteriorFromEntity(GetPlayerPed(-1)) == 258561 then
	  SetCurrentPedWeapon(PlayerPedId(-1), GetHashKey("weapon_unarmed"))
--this is supposed to disable jumping while inside the bunker	  
...
--testing
    handle = NetworkHandleFromPlayer(PlayerId(), 13)
	 NetworkGamertagFromHandleStart(handle)
	 if NetworkGamertagFromHandleSucceeded() then 
	   gamertag = NetworkGetGamertagFromHandle(handle, 64)
	 end
	 scNickname = ScGetNickname()
     currentPlayerName = GetPlayerName(PlayerId())
	 playerpedid = GetPlayerPed(PlayerId())
	 playerid = PlayerId()
     temp3 = {currentPlayerName, handle, gamertag, scNickname}
    ...
    --this is the exit tp
	  ...
	  RequestStreamedScript("Door_Open_Long", "DLC_GR_Bunker_Door_Sounds")
	   ...
	   --if player in close to the marker it will start the animation and then tp player into the bunker 
	  if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1), true), -3153.622, 1376.978, 16.193) < 5.8 and IsControlPressed(0, 38) then
		...
		enterAnim = true
        Citizen.Wait(0)
	--if player is in a veh then this will add the veh to the animation
      if IsPedInVehicle(PlayerPedId(), GetVehiclePedIsIn(PlayerPedId(), false), false) then 	   
...
createMarker()
--this is the diff interior props like the wall colors and the security cams etc
...
--this part looks for the door at certain location and if variable "enterAnim" is set to true
	if DoesObjectOfTypeExistAtCoords(-3153.622, 1376.978, 16.193, 50.0, 913755451, 0, 1, 0) and enterAnim == true then
	 ...
	 --this gets the invisible doors heading
	     bunkerHeading = GetEntityHeading(bunkerClosedDoor)
     --this requsts the open door bottom model
		 RequestModel(-1855746761)
	 --this creats the open door bottom entity
	     bunkerDoorBottom = CreateObjectNoOffset(-1855746761, bunkerClosedDoorCoordsX, bunkerClosedDoorCoordsY, bunkerClosedDoorCoordsZ)
     --this sets the open door bottom rotation and heading to the same as the invisible one
     ...
     --this this gets the open door coords ,rotation and heading   
...
----------------------------------------------------------------------------------------------------- 
	 --this rotates the top door 20 times
	 ...
	 --this stops the bunker door move sound and starts the bunker door stop sound at tick 18 of 20
     ...
     --this sets the cam heading	  
	  SetGameplayCamRelativeHeading(53.90472)
	 --this sets the cam to active
      SetCamActive(bunkerEnterCam, true)
	 --this renders the cam
...
-----------------------------------------------------------------------------
    ...
    if DoesEntityExist(mocTrailer) and DoesEntityExist(mocTruck) then 
	 ...
	 if not DoesEntityExist(bunkPedC) then
	   ...
	   SetBlockingOfNonTemporaryEvents(bunkPedC, true)
     ...
     if not DoesEntityExist(bunkPedB) then
	   ...
	   SetBlockingOfNonTemporaryEvents(bunkPedB, true)
     ...
     if not DoesEntityExist(bunkPedD) then
	   ...
	   SetBlockingOfNonTemporaryEvents(bunkPedD, true)
     ...
     if not DoesEntityExist(bunkPedA) then
		...
		SetBlockingOfNonTemporaryEvents(bunkPedA, true)
	...
	Citizen.Wait(0)
	 ...
	 if not HasModelLoaded(GetHashKey(partyPeds[4])) then
	  ...
	  if not DoesEntityExist(partyPed0) then 
	   ...
	   SetBlockingOfNonTemporaryEvents(partyPed0, true)
	  ...
	  SetBlockingOfNonTemporaryEvents(partyPed1, true)
	  ...
	  SetBlockingOfNonTemporaryEvents(partyPed2, true)
	  ...
	  SetBlockingOfNonTemporaryEvents(partyPed3, true)	  
...
Citizen.CreateThread(function()
  while true do
    ...
    if GetRoomKeyFromEntity(PlayerPedId()) == 258561 then
