...
function logInButton() 		  
 if HasNamedScaleformMovieLoaded("DISRUPTION_LOGISTICS") then
      PushScaleformMovieFunction(scaleform, "ACTIVATE")
	PopScaleformMovieFunctionVoid()
  ...
  selectionBool = Citizen.InvokeNative(0x768FF8961BA904D6, getCurrentSelection)
   ...
   nVar = Citizen.InvokeNative(0x2DE7EFA66B906036, getCurrentScreenID)
	...
	PushScaleformMovieFunction(scaleform, "APP_IS_DEACTIVATED")
	 ...
	 Citizen.Trace("vVar "..tostring(vVar))	
...
if IsControlJustReleased(2, 237) and vVar == nil then
 CallScaleformMovieFunctionFloatParams(scaleform, "SET_INPUT_RELEASE_EVENT")
 PopScaleformMovieFunctionVoid()
end  
  ...
  if selectionBool --[[and ScreenIDBool--]] then 
   ...
   --setDisruptionResearchStats()
     ...
     if vVar == 16 and nVar == 4 then
	  ...
	  BeginTextComponent("UA_MODE_S_FAIL4")
	  EndTextComponent()
	  BeginTextComponent("WHOUSE_CONF")
	  EndTextComponent()
	  BeginTextComponent("WHOUSE_CANC")
	  EndTextComponent()
	  PopScaleformMovieFunctionVoid()
      staff = 0
   end
     if vVar == 1 and nVar == 4 then
      PushScaleformMovieFunction(scaleform, "HIDE_OVERLAY")
	  PopScaleformMovieFunctionVoid()
      staff = 0
   end
...
Citizen.CreateThread(function()
 while true do
  Citizen.Wait(0)
     ...
     playerInterior = GetInteriorFromEntity(playerPed)
	 if playerInterior == 258561 and (disruptScreen == false or disruptScreen == nil) then
    ...
    if playerInterior == 258561 and disruptScreen == true then
